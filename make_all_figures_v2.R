...
# Ensure the order of factors
lt_circ_prop_summary$Animal <- factor(lt_circ_prop_summary$Animal, levels = c("DGA0 6hr", "DGF5 6hr","G45T 6hr","G45T 48hr"))
...
pdf(file = file.path(results_dir, "PB_long_term_circulating_percentages_with_T_cells.pdf"), width = 7, height = 4)
plot(p)
dev.off()
...
means_6hr <- read.csv(file = file.path(data_dir, "PB_6hr_means_norm_T.csv"))
...
# Dot plot with pairwise wilcoxon test to compare means
...
p <- ggplot(means_6hr, aes(x = Cell_Type, y = value, fill = Cell_Type)) +
  geom_line(aes(x = Cell_Type, y = value, group = Animal)) +
 #  geom_dotplot(binaxis='y', stackdir='center', dotsize = 2) +
  geom_jitter(size = 4, position=position_dodge(0.8), aes(colour = Cell_Type)) + 
  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = c(5, 5.5, 6), paired = T) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  labs(title = "", y = "Fold Change versus T Cells") + 
...
pdf(file = file.path(results_dir, "PB_6hour_proportions_normalized_dotplot_norm_T.pdf"), width = 4, height = 4)
plot(p)
dev.off()
...
PB_data_long$Animal <- factor(PB_data_long$Animal, levels = c("DGA0", "DGF5", "G45T"))
...
# Read the data
LN_ivas_prop <- read.csv(file = file.path(data_dir, 'LN_IVAS_positive_proportions_updated.csv'))
...
pdf(file = file.path(results_dir, "LN_IVAS_positive_percentages_with_T.pdf"), width = 10, height = 4)
plot(p)
dev.off()

## LN IVAS+ summary dot plot 6hr ###################################################
# Load means normalized to T cell
means_ivas <- read.csv(file = file.path(data_dir, "LN_IVAS_positive_proportions_norm_T.csv"))
...
p <- ggplot(means_ivas, aes(x = Cell_Type, y = value, fill = Cell_Type)) +
  geom_line(aes(x = Cell_Type, y = value, group = Animal)) +
  # geom_dotplot(binaxis='y', stackdir='center', dotsize = 2) + theme_classic() +
  geom_jitter(size = 4, position=position_dodge(0.8), aes(colour = Cell_Type)) + 
  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = c(log10(100), log10(150), log10(250)), paired = T) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  scale_y_continuous(trans = "log10") +
  labs(y = "Fold Change versus T Cells (log)") + 
...
pdf(file = file.path(results_dir, "LN_IVAS_positive_dotplot_norm_T.pdf"), width = 4, height = 4)
plot(p)
dev.off()
...
# Read the data
LN_tr_prop <- read.csv(file = file.path(data_dir, 'LN_tissue_resident_proportions_updated.csv'))
...
pdf(file = file.path(results_dir, "LN_tissue_resident_percentages_with_T.pdf"), width = 10, height = 4)
plot(p)
dev.off()
...
# Load means normalized to T cell
means_tr <- read.csv(file = file.path(data_dir, "LN_tissue_resident_proportions_norm_T.csv"))
...
p <- ggplot(means_tr, aes(x = Cell_Type, y = value, fill = Cell_Type)) +
  ...
  geom_jitter(size = 4, position=position_dodge(0.8), aes(colour = Cell_Type)) + 
  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = c(1.2, 1.225, 1.25), paired = T) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  labs(y = "Fold Change versus T Cells") + 
  theme(title = element_text(size = 14, color = "black"),
        plot.title = element_blank(),
        axis.title.y = element_text(size = 14, color = "black", margin = margin(t = 0, r = 10, b = 0, l = 0)),
        axis.title.x = element_blank(),
...
pdf(file = file.path(results_dir, "LN_tissue_residence_dotplot_norm_T.pdf"), width = 4, height = 4)
plot(p)
dev.off()
...
# Read the data
LN_entry_prop <- read.csv(file = file.path(data_dir, 'LN_entry_rate_proportions_6hr_updated.csv'))
...
p <- ggplot(LN_entry_prop_summary, aes(x=Animal, y=value, fill=`Cell Type`)) + 
  ...
  theme(axis.title.x  = element_blank(), axis.title.y  = element_text(size = 14, color = "black", margin = margin(t = 0, r = 10, b = 0, l = 0)),
        ...
        legend.title = element_text(size = 12, color = "black", face = "bold"), legend.text = element_text(size = 10, color = "black"))

pdf(file = file.path(results_dir, "LN_entry_rates_6hr_with_T.pdf"), width = 10, height = 4)
plot(p)
dev.off()
...
means_LN_entry <- read.csv(file = file.path(data_dir, "LN_entry_rate_means_norm_T.csv"))
...
p <- ggplot(means_LN_entry, aes(x = Cell_Type, y = value, fill = Cell_Type)) +
  ...
  geom_jitter(size = 4, position=position_dodge(0.8), aes(colour = Cell_Type)) + 
  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = c(1.75, 1.8725, 2), paired = T) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  labs(y = "Fold Change versus T Cells") + 
  theme(title = element_text(size = 14, color = "black"),
        plot.title = element_blank(),
...
pdf(file = file.path(results_dir, "LN_entry_rate_normalized_dotplot_with_T.pdf"), width = 4, height = 4)
plot(p)
dev.off()
...
# Ensure the order of my factors
LN_data_long$Animal <- factor(LN_data_long$Animal_Sample, levels = c("DGA0 AxLN", "DGA0 MesLN", "DGF5 AxLN", "G45T AxLN", "G45T IngLN"))
...
# Read data
LN_pheno_data <- read.csv(file = file.path(data_dir, "LN_phenotype_data_updated.csv"))
...
for (my_animal in unique(LN_pheno_data$Animal)){
  ...
  pdf(file = file.path(results_dir, paste(my_animal, "NK_phenotype_pie_chart_updated.pdf", sep = "_")), width = 5*3/2, height = 2.5)
...
p <- ggplot(LN_pheno_melt, aes(x = Tissue, y = value)) +
  facet_wrap(~variable)+
  geom_line(aes(x = Tissue, y = value, group = Animal)) +
  # geom_dotplot(binaxis='y', stackdir='center', dotsize = 1.5, position=position_dodge(0.8)) +  theme_classic() +
  geom_jitter(size = 3, position=position_dodge(0.8), aes(colour = variable)) + 
  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = c(100, 107, 114), paired = T) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
...
# Save to PDF
pdf(file = file.path(results_dir, "LN_phenotype_summary_dotplot_updated.pdf"), width = 6, height = 5)
plot(p)
dev.off()
...
## Spleen Phenotype summary plot
# Read data
Spleen_pheno_data <- read.csv(file = file.path(data_dir, "Spleen_phenotype_data.csv"))

# Rename tissue variable
Spleen_pheno_data$Tissue <- plyr::revalue(Spleen_pheno_data$Tissue, c("PB" = "Blood", "Spleen IVAS_pos" = "Spleen IVas+", "Spleen IVAS_neg" = "Spleen IVas-") )

# Turn data into percentage out of 100
Spleen_pheno_data[,3:5] <- Spleen_pheno_data[,3:5] / rowSums(Spleen_pheno_data[,3:5]) * 100

# Melt
Spleen_pheno_melt <- reshape2::melt(Spleen_pheno_data, id = c("Animal","Tissue"))

# Replace _ with space in subset name
Spleen_pheno_melt$variable <- gsub("_", " ", Spleen_pheno_melt$variable)
Spleen_pheno_melt$variable <- gsub("CD56 NK", "CD56+ NK", Spleen_pheno_melt$variable)
Spleen_pheno_melt$variable <- gsub("CD16 NK", "CD16+ NK", Spleen_pheno_melt$variable)
Spleen_pheno_melt$variable <- factor(Spleen_pheno_melt$variable, levels = c("CD56+ NK", "DN NK", "CD16+ NK"))

# Keep only the desired data
Spleen_pheno_melt <- Spleen_pheno_melt[Spleen_pheno_melt$Tissue %in% c("Blood", "Spleen IVas+", "Spleen IVas-"),]

# Ensure order of factors
Spleen_pheno_melt$Tissue <- factor(Spleen_pheno_melt$Tissue, levels = c("Blood", "Spleen IVas+", "Spleen IVas-"))

# Set comparisos list for statistics
my_comparisons <- list( c("Blood", "Spleen IVas+"), c("Spleen IVas+", "Spleen IVas-"), c("Blood", "Spleen IVas-") )

# Dot plot with statistics
p <- ggplot(Spleen_pheno_melt, aes(x = Tissue, y = value)) +
  facet_wrap(~variable)+
  geom_line(aes(x = Tissue, y = value, group = Animal)) +
  # geom_dotplot(binaxis='y', stackdir='center', dotsize = 1.5, position=position_dodge(0.8)) +  theme_classic() +
  geom_jitter(size = 3, position=position_dodge(0.8), aes(colour = variable)) + 
...
# Save to PDF
pdf(file = file.path(results_dir, "Spleen_phenotype_summary_dotplot_updated.pdf"), width = 6, height = 5)
plot(p)
dev.off()
...
## Comparison of KI-67+ percentage between LN RI and TR
# Read data
tissue_comp_data <- read.csv(file = file.path(data_dir, "RI_vs_TR_KI67_percentages.csv"))
...
# Dot plot with coloring
p <- ggplot(tissue_comp_data, aes(x = Subset, y = KI67)) +
  facet_wrap(~Cell_Type)+
  geom_line(aes(x = Subset, y = KI67, group = Animal)) +
#  geom_dotplot(binaxis='y', stackdir='center', dotsize = 1, position=position_dodge(0.8)) +  theme_classic() +
  geom_jitter(size = 3, position=position_dodge(0.8), aes(colour = Cell_Type)) +  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = 42, paired = T) + 
  scale_colour_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  labs(title = "", y = "Percentage KI-67+") + 
  theme(title = element_blank(),
        plot.title = element_blank(),
        axis.title.y = element_text(size = 12, color = "black", margin = margin(t = 0, r = 10, b = 0, l = 0)),
        axis.title.x = element_blank(),
...
# Save to PDF
pdf(file = file.path(results_dir, "RI_vs_TR_KI67_percentage_dotplot.pdf"), width = 6, height = 5)
plot(p)
dev.off()


## Comparison of CD49a+ percentage between LN RI and TR
# Read data
tissue_comp_data <- read.csv(file = file.path(data_dir, "RI_vs_TR_CD49a_percentages.csv"))
...
p <- ggplot(tissue_comp_data, aes(x = Subset, y = percentage)) +
  facet_wrap(~Cell_Type)+
  geom_line(aes(x = Subset, y = percentage, group = Animal)) +
  #  geom_dotplot(binaxis='y', stackdir='center', dotsize = 1, position=position_dodge(0.8)) +  theme_classic() +
  geom_jitter(size = 3, position=position_dodge(0.8), aes(colour = Cell_Type)) +  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = 27, paired = T) + 
  scale_colour_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  labs(title = "", y = "Percentage CD49a+") + 
  theme(title = element_blank(),
        plot.title = element_blank(),
        axis.title.y = element_text(size = 12, color = "black", margin = margin(t = 0, r = 10, b = 0, l = 0)),
        axis.title.x = element_blank(),
...
# Save to PDF
pdf(file = file.path(results_dir, "RI_vs_TR_CD49a_percentage_dotplot.pdf"), width = 6, height = 5)
plot(p)
dev.off()


## Comparison of CD69+ percentage between LN RI and TR
# Read data
tissue_comp_data <- read.csv(file = file.path(data_dir, "RI_vs_TR_CD69_percentages.csv"))
...
p <- ggplot(tissue_comp_data, aes(x = Subset, y = percentage)) +
  ...
  #  geom_dotplot(binaxis='y', stackdir='center', dotsize = 1, position=position_dodge(0.8)) +  theme_classic() +
  geom_jitter(size = 3, position=position_dodge(0.8), aes(colour = Cell_Type)) +  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = 102, paired = T) + 
  scale_colour_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  labs(title = "", y = "Percentage CD69+") + 
  theme(title = element_blank(),
        plot.title = element_blank(),
        axis.title.y = element_text(size = 12, color = "black", margin = margin(t = 0, r = 10, b = 0, l = 0)),
        axis.title.x = element_blank(),
...
# Save to PDF
pdf(file = file.path(results_dir, "RI_vs_TR_CD69_percentage_dotplot.pdf"), width = 6, height = 5)
plot(p)
dev.off()


## Comparison of CXCR3+ percentage between LN RI and TR
# Read data
tissue_comp_data <- read.csv(file = file.path(data_dir, "RI_vs_TR_CXCR3_percentages.csv"))
...
p <- ggplot(tissue_comp_data, aes(x = Subset, y = percentage)) +
  ...
  #  geom_dotplot(binaxis='y', stackdir='center', dotsize = 1, position=position_dodge(0.8)) +  theme_classic() +
  geom_jitter(size = 3, position=position_dodge(0.8), aes(colour = Cell_Type)) +  theme_classic() +
  stat_compare_means(comparisons = my_comparisons, method = "t.test", label.y = 82, paired = T) + 
  scale_colour_manual(values = RColorBrewer::brewer.pal(4, "Set1")[c(4,3,2)]) +
  labs(title = "", y = "Percentage CXCR3+") + 
  theme(title = element_blank(),
        plot.title = element_blank(),
        axis.title.y = element_text(size = 12, color = "black", margin = margin(t = 0, r = 10, b = 0, l = 0)),
        axis.title.x = element_blank(),
...
# Save to PDF
pdf(file = file.path(results_dir, "RI_vs_TR_CXCR3_percentage_dotplot.pdf"), width = 6, height = 5)
plot(p)
dev.off()


## Heatmap of Cont. Circulating versus Non Cont. Circulating PB NK cells
# Load the data
PB_phenotype_FC <- read.csv(file = file.path(data_dir, "PB_phenotype_log_fold_changes.csv"))

# Select desired markers
PB_phenotype_FC <- PB_phenotype_FC[, c("Animal", "Subset",
                                       "CD49a","CD161", "NKp46", "CCR7", "CD69", "CXCR3", "KI67")]

# Set factor orders
PB_phenotype_FC$Subset <- factor(PB_phenotype_FC$Subset, levels = c("CD56", "DN", "CD16"))
PB_phenotype_FC$Animal <- factor(PB_phenotype_FC$Animal, levels = c("DGA0", "DGF5", "G45T"))

# Order first by subset then by animal
PB_phenotype_FC <- PB_phenotype_FC[order(PB_phenotype_FC$Subset, PB_phenotype_FC$Animal),]

# Put into matrix
PB_phenotype_FC_mat <- t(as.matrix(PB_phenotype_FC[,3:ncol(PB_phenotype_FC)]))
colnames(PB_phenotype_FC_mat) <- PB_phenotype_FC$Animal
range <- max(abs(PB_phenotype_FC_mat), na.rm = T)

# Make heatmap
pdf(file = file.path(results_dir, "PB_phenotype_CC_vs_non_CC.pdf"), width = 6, height = 5.5)
pheatmap(PB_phenotype_FC_mat,
...
## Heatmap of RI versus TR LN NK cells
# Load the data
LN_phenotype_FC <- read.csv(file = file.path(data_dir, "LN_phenotype_log_fold_changes.csv"))

# Select desired markers
LN_phenotype_FC <- LN_phenotype_FC[, c("Animal", "Subset",
                                       "CD49a","CD161", "NKp46", "CCR7", "CD69", "CXCR3", "KI67")]

# Set factor orders
LN_phenotype_FC$Subset <- factor(LN_phenotype_FC$Subset, levels = c("CD56", "DN", "CD16"))
LN_phenotype_FC$Animal <- factor(LN_phenotype_FC$Animal, levels = c("DGA0 AxLN", "DGA0 MesLN", "DGF5 AxLN",
                                                                    "G45T AxLN", "G45T IngLN"))

# Order first by subset then by animal
LN_phenotype_FC <- LN_phenotype_FC[order(LN_phenotype_FC$Subset, LN_phenotype_FC$Animal),]

# Put into matrix
LN_phenotype_FC_mat <- t(as.matrix(LN_phenotype_FC[,3:ncol(LN_phenotype_FC)]))
colnames(LN_phenotype_FC_mat) <- LN_phenotype_FC$Animal
range <- max(abs(LN_phenotype_FC_mat), na.rm = T)

# Make heatmap
pdf(file = file.path(results_dir, "LN_phenotype_RI_vs_TR.pdf"), width = 9, height = 6)
pheatmap(LN_phenotype_FC_mat,
         cluster_rows = FALSE, cluster_cols = FALSE,
         show_rownames = T, show_colnames = T,
         breaks = seq(-range, range, length.out = 100),
         na_col = "grey75") 
...
## HAXR Bar plot of long term circulating proportions ###################################################
# Read the data
lt_circ_prop <- read.csv(file = file.path(data_dir, 'PB_long_term_circulating_proportions_with_T_HAXR.csv'))
...
# Ensure the order of factors
lt_circ_prop_summary$Animal <- factor(lt_circ_prop_summary$Animal, levels = c("G45T 6hr","G45T 48hr", "HAXR 6hr", "HAXR 48hr"))
...
pdf(file = file.path(results_dir, "PB_long_term_circulating_percentages_with_T_cells_HAXR.pdf"), width = 8.5, height = 4)
plot(p)
dev.off()
...
# Read the data
LN_tr_prop <- read.csv(file = file.path(data_dir, 'LN_tissue_resident_proportions_HAXR_vs_mean.csv'))

# Include only CD56 and DN NK cells
# LN_tr_prop <- LN_tr_prop[LN_tr_prop$Cell_Type %in% c("T Cell", "CD56 NK", "DN NK"),]

# Ensure the order of factors
LN_tr_prop$Group <- factor(LN_tr_prop$Group, levels = c("LN Mean", "HAXR AxLN"))
LN_tr_prop$Cell_Type <- gsub("CD16 NK", "CD16+ NK", LN_tr_prop$Cell_Type)
LN_tr_prop$Cell_Type <- gsub("CD56 NK", "CD56+ NK", LN_tr_prop$Cell_Type)
...
pdf(file = file.path(results_dir, "LN_tissue_resident_percentages_HAXR_vs_SS_mean.pdf"), width =6, height = 4)
plot(p)
dev.off()
...
# Read the data
LN_entry_prop <- read.csv(file = file.path(data_dir, 'LN_entry_rate_proportions_48hr_6hr_updated_HAXR.csv'))
...
# Ensure the order of factors
LN_entry_prop$Cell_Type <- gsub("CD56 NK", "CD56+ NK", LN_entry_prop$Cell_Type)
LN_entry_prop$Cell_Type <- gsub("CD16 NK", "CD16bright NK", LN_entry_prop$Cell_Type)
...
LN_entry_prop$Cell_Type <- factor(LN_entry_prop$Cell_Type, levels = c("T Cell","CD56+ NK", "DN NK", "CD16dim NK", "CD16bright NK"))

# Include only -48 hour calculated rates
LN_entry_prop <- LN_entry_prop[LN_entry_prop$Animal %in% c("G45T AxLN -48hr", "G45T IngLN -48hr", "HAXR AxLN -48hr"),]
...
pdf(file = file.path(results_dir, "LN_entry_rates_48hr_HAXR_with_T.pdf"), width = 7.5, height = 4)
plot(p)
dev.off()
...
# Read the data
LN_ivas_prop <- read.csv(file = file.path(data_dir, 'LN_IVAS_positive_proportions_HAXR_vs_mean.csv'))

# # Exclude lymphocytes
# LN_ivas_prop <- LN_ivas_prop[LN_ivas_prop$Cell_Type %in% c("CD56 NK", "DN NK", "CD16 NK"),]

# Ensure the order of factors
LN_ivas_prop$Group <- factor(LN_ivas_prop$Group, levels = c("LN Mean", "HAXR AxLN"))
LN_ivas_prop$Cell_Type <- gsub("CD16 NK", "CD16+ NK", LN_ivas_prop$Cell_Type)
LN_ivas_prop$Cell_Type <- gsub("CD56 NK", "CD56+ NK", LN_ivas_prop$Cell_Type)
...
pdf(file = file.path(results_dir, "LN_IVAS_positive_percentages_HAXR_vs_SS_mean.pdf"), width = 6, height = 4)
plot(p)
dev.off()